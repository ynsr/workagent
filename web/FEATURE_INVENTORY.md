...
| `cleanup REF` | mutating | — | `cleanup:<ref>` | **yes** (closes tracker issue, removes worktree + local branch, closes/merges PR; remote branch deleted only on merge of the branch PR, `--force` merges-then-closes and always deletes it) | no linked state; conflicted/404 PR keeps remote + link (1) | none (non-TTY) / y-N (TTY) |
